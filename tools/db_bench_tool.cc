...
DEFINE_bool(experimental_use_data_block_skiplist_index, false,
            "Experimental: append a skiplist-ish data block index (and store "
            "it on disk) to reduce restart-key decoding/comparisons for large "
            "data blocks. Requires rebuilding SSTs (i.e., re-fill DB) for the "
            "format to take effect.");

DEFINE_uint64(experimental_macro_block_bytes, 0,
              "Experimental: if non-zero and "
              "experimental_use_data_block_skiplist_index is true, override "
              "BlockBasedTableOptions::block_size with this value (e.g. 16384 "
              "for 16KB macro-blocks).");
...
DEFINE_string(sample_probe_output, "",
              "If non-empty, append periodically-sampled seek perf deltas to "
              "this CSV file. The CSV format matches --tail_probe_output.");
DEFINE_int64(
    sample_probe_interval_ops, 0,
    "Record one sample every N seek ops per thread. Samples use per-op PerfContext/"
    "IOStatsContext deltas (like tail probe), but selection is periodic rather "
    "than latency-threshold-based. <=0 disables sampling.");
...
class MixGraphCacheSampler {
 ...
 private:
  ...
  void FlushStage(bool force) {
    ...
    // Flush last stage on transition or force.
    const Snapshot cur = CaptureSnapshot();
    ...
    stage_out_ << last_stage_id_ << "," << stage_start_us_ << "," << now_us
               ...
               << d.add_failures << "," << d.burst_scans << "," << d.burst_entries
               << "," << d.probe_ops << "," << d.probe_hit << ","
               << d.probe_incomplete << "," << d.probe_notfound << ","
               << d.probe_error << "\n";
    stage_out_.flush();
...
static inline bool SupportsSampleSeekProbe(const Slice& bench_name) {
  // Currently we only sample seek ops; restrict to benchmarks that execute Seek.
  return SupportsTailSeekProbe(bench_name);
}
...
class Stats {
 private:
  ...
  uint64_t sample_probe_interval_seek_ops_ = 0;
  ...
  uint64_t sample_probe_seek_ops_ = 0;
 ...
 public:
  ...
  void FinishedOps(DBWithColumnFamilies* db_with_cfh, DB* db, int64_t num_ops,
    ...
    if (tail_probe_enabled_ || sample_probe_enabled_) {
      ...
      if (sample_probe_enabled_) {
        if (op_type == kSeek) {
          ++sample_probe_seek_ops_;
          const bool do_sample =
              sample_probe_interval_seek_ops_ > 0 &&
              (sample_probe_seek_ops_ % sample_probe_interval_seek_ops_ == 0);
          if (do_sample && sample_probe_snapshot_initialized_ &&
            ...
            const std::string& scenario =
                sample_probe_scenario_.empty() ? std::string("seek_sample")
...
struct SharedState {
  ...
  uint64_t sample_probe_interval_seek_ops = 0;
 ...
 private:
  ...
  static void ThreadBody(void* v) {
    ...
    thread->stats.ConfigureSampleProbe(shared->sample_probe_writer,
                                       shared->sample_probe_interval_seek_ops,
  ...
  void InitializeOptionsFromFlags(Options* opts) {
    ...
    if (FLAGS_use_plain_table) {
      ...
      if (FLAGS_use_data_block_hash_index &&
          FLAGS_experimental_use_data_block_skiplist_index) {
      ...
      } else if (FLAGS_experimental_use_data_block_skiplist_index) {
        block_based_options.data_block_index_type =
            ROCKSDB_NAMESPACE::BlockBasedTableOptions::
      ...
      if (FLAGS_experimental_use_data_block_skiplist_index &&
          FLAGS_experimental_macro_block_bytes != 0) {
        block_based_options.block_size = FLAGS_experimental_macro_block_bytes;
      }
...
// optimization, so skip the check of the max level.
  ...
  class GenerateTwoTermExpKeys {
   public:
    ...
    void SetShiftBaseUsecs(uint64_t base_usecs) {
      ...
      // If a shift state was already initialized, keep its current stage/layout
      // and only adjust epoch if it has not been set. This is best-effort and
      // primarily for aligning stage boundaries across threads.
      if (!shift_inited_) {
    ...
    int64_t DistGetColdKeyID(int64_t ini_rand) {
      if (keyrange_num_ <= 0 || keyrange_size_ <= 0 ||
          keyrange_set_.empty()) {
        return 0;
      }
      ...
      if (FLAGS_mix_hotset_enable && FLAGS_mix_burst_cold_ranges_only) {
        // Try a few pseudo-random key ranges and pick the first cold one.
      ...
      if (picked < 0) {
        picked = static_cast<int64_t>(rand_a % static_cast<uint64_t>(keyrange_num_));
      }
    ...
    int64_t DistGetHotKeyID(int64_t ini_rand, double key_dist_a,
                            double key_dist_b) {
      if (!FLAGS_mix_hotset_enable || keyrange_num_ <= 0 || keyrange_size_ <= 0 ||
          keyrange_set_.empty() || hot_count_ <= 0) {
      ...
      const int64_t hot_rank = static_cast<int64_t>(
          rand_a % static_cast<uint64_t>(std::max<int64_t>(1, hot_count_)));
  ...
  void MixGraph(ThreadState* thread) {
    ...
    if (FLAGS_mix_shift_enable && event_writer != nullptr &&
      ...
      // Log stage-0 layout once for correlation with time series.
      event_writer->LogEvent(FLAGS_env->NowMicros(), "shift_stage", thread->tid,
                             /*stage=*/0, /*range_id=*/gen_exp.hot_window_start(),
                             /*key_id=*/-1,
    ...
    while (!duration.Done(1)) {
      ...
      // Start the query
      if (do_burst) {
        // Simulate a bursty scan workload that is likely to miss in block cache:
        // seek to a cold key range then scan a fixed number of entries.
        ...
        if (use_prefix_modeling && FLAGS_mix_hotset_enable &&
          ...
          // Best-effort: pick a far-ish key to reduce overlap with the hot set.
          if (FLAGS_num > 0) {
        ...
        if (event_writer != nullptr && event_writer->Enabled()) {
          ...
          if (use_prefix_modeling && FLAGS_keyrange_num > 0) {
            range_id = burst_key_rand / std::max<int64_t>(1, (FLAGS_num / FLAGS_keyrange_num));
          }
          event_writer->LogEvent(FLAGS_env->NowMicros(), "burst_scan", thread->tid,
                                 monitor_shared ? monitor_shared->shift_stage.load() : 0,
        ...
        if (FLAGS_mix_burst_log) {
          fprintf(stderr,
                  "mix_burst tid=%d op=%" PRId64 " key=%" PRId64
                  " nexts=%d\n",
        ...
        if (db_with_cfh->db != nullptr) {
          for (int32_t i = 0; i < multiget_batch; ++i) {
            ...
            if (use_prefix_modeling) {
              ...
              int64_t seed = PowerCdfInversion(uu, FLAGS_key_dist_a, FLAGS_key_dist_b);
              Random64 rr(seed);
          ...
          if (FLAGS_num_column_families > 1) {
            ...
            for (int32_t i = 0; i < multiget_batch; ++i) {
              ...
              if (multiget_statuses[idx].ok()) {
                multiget_found++;
                bytes += multiget_key_slices[idx].size() + multiget_vals[idx].size();
