...
# Exp49: Gate2 noise reduction via mmap-base-IO (DAX-like) while keeping
# simulated delay active.
#
# Reuses the existing exp46 DB/tmpfs root by default and runs a 4-point
# xp_latency sweep with:
...
GLOBAL_EXP_ID="${GLOBAL_EXP_ID:-49}"
EXPERIMENT_NAME="${EXPERIMENT_NAME:-gate2_mmap_base_io_sweep}"
...
# Reuse DB from exp46 by default.
...
# DB shape (must match the existing DB).
...
for xp_ns in "${XP_ARRAY[@]}"; do
  ...
  run_tag="${EXP_DATE}_exp${GLOBAL_EXP_ID}_${EXPERIMENT_NAME}_xplat${xp_ns}_mmap1_${RUN_TIME}"
...
echo "[exp49] done: $EXPERIMENT_DIR/sweep_summary.csv"
